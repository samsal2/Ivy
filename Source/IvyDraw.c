...
static void ivyBindVertexGraphicsTemporaryBuffer(
...
static void ivyBindIndexGraphicsTemporaryBuffer(
    IvyRenderer                *renderer,
...
static void ivyBindUniformGraphicsTemporaryBufferAndTexture(
  ...
  vkCmdBindDescriptorSets(
      ...
      0,
      IVY_ARRAY_LENGTH(descriptorSets),
      descriptorSets,
      1,
      &offset);
}
...
IvyCode ivyDrawRectangle(
  ...
  ivyBindUniformGraphicsTemporaryBufferAndTexture(
      renderer,
      &uniformBuffer,
      texture);

  vkCmdDrawIndexed(frame->commandBuffer, IVY_ARRAY_LENGTH(indices), 1, 0, 0, 0);
